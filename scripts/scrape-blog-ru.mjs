...
const OUTPUT_DIR = join(__dirname, '..', 'src', 'content', 'blogi-ru');
const URLS_FILE = join(__dirname, 'blog-urls-ru.json');
...
async function main() {
  ...
  await writeFile(join(__dirname, 'scrape-summary-ru.json'), JSON.stringify({ total: results.length, errors }, null, 2));
